...
def main():
    ...
    bool = items_data["商家编码"].str.contains('8181')
    ...
    frequent_itemsets = apriori(items_data, min_support=0.005, use_colnames=True)
    rules = association_rules(frequent_itemsets, metric="lift", min_threshold=1)
    #antecedants前项集，consequents后项集，support支持度，confidence置信度，lift提升度。
    rules = rules[ (rules['lift'] >= 6) &(rules['confidence'] >= 0.35) ].sort_values("lift",ascending = False)
    print(rules.head())
